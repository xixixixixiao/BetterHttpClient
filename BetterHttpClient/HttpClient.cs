...
namespace BetterHttpClient
{
    public class HttpClient : WebClient
    {
        ...
        protected override WebRequest GetWebRequest(Uri address)
        {
            ...
            {
                if (request is SocksHttpWebRequest socksRequest)
                {
        ...
        byte[] Download(string url, NameValueCollection data, string dataStr)
        {
            ...
            var          unkownProxy      = Proxy.ProxyType == ProxyTypeEnum.Unknown;

            while (counter <
                   NumberOfAttempts +
                   (NumberOfAttempts < 2 && unkownProxy ? 1 : 0)) // min two try for unkonwn proxy type
            {
                try
                {
